...
## Repo Shape
- This is a stable Python package with package metadata in `pyproject.toml`.
- The distribution name is `2ning` and the import package is `tuning`.
...
## Naming conventions
- The distribution name is `2ning`.
...
- The current runtime roadmap is stabilized for the 1.0 release. Treat grouped boxes as a follow-up feature, not current implementation scope.
...
## Roadmap
- Stabilization pass is complete for the 1.0 release; keep adding pytest coverage around behavior changes.
