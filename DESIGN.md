...
This project is a stable 1.0 package and is now back in development for 1.1.
The PyPI distribution is `2ning`; the import package remains `tuning`.
...
Examples:
- `examples/usage.py`: runnable usage example with selectable config modes
...
- release metadata includes MIT license, author, documentation URL, repository URL, and issue tracker URL
- build tooling is included in the `dev` extra
- editable install behavior has been verified with all extras

Publishing uses the release workflow with PyPI Trusted Publishing.
...
- [X] Upgrade version to 1.0 and prepare for publishing
- Version 1.1:
  - [ ] Add README/docs screenshots for representative console output and YAML configuration.
  - [ ] Evaluate [`rich-gradient`](https://github.com/maxludden/rich-gradient) as an optional dependency before adding any public wrapper.
  - [ ] In panel/banner rendering, evaluate optional metadata display:
    - [ ] Optional, hidden by default: show version from package metadata.
    - [ ] Optional, hidden by default: show author from package metadata.
    - [ ] Optional, hidden by default: show a footer note from explicit configuration.
